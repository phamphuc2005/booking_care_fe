...
class ModalDeleteClinic extends Component {
    ...
    handleDeleteClinic = () => {
        this.props.deleteClinicModal(this.state);
    }
    ...
    render() {
        return (
            <Modal 
                ...
                size="lg"
            >
                <ModalHeader toggle={()=>{this.toggle()}}><FormattedMessage id = "clinic-modal.delete-title"/></ModalHeader>
                <ModalBody>
                    <div><FormattedMessage id = "clinic-modal.delete-content"/></div>
                </ModalBody>
                <ModalFooter>
                    <Button 
                        color="danger px-3" 
                        onClick={()=>{this.handleDeleteClinic()}}
                    ><FormattedMessage id = "clinic-modal.delete-btn"/></Button>{' '}
